...
export class SigninComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if(timeline) {
      ...
      setTimeout(() => {
        ls.set('canloginAsAdmin', true, { encrypt: true});
  ...
  */
  ...
  async onSubmit() {
    ...
    const docRef = doc(this.db, 'HealusAssuranceAdminUsers', `${ this.signInForm.value.email_address }`);
    const docSnap = await getDoc(docRef);

    if (docSnap.exists()) {
      this._authService.pangeaLoginWithPassword(dataSet).subscribe((result: any) => {
      ...
      this._snackBar.open('Not Authorized To Log In As Admin', 'Close');
